 ...
 * Uses CSS data attributes to avoid conflicts with next-themes
 */
export function applyTheme(theme: ThemeConfig): void {
  try {
    ...
    // This allows CSS to handle theming via cascade, not inline styles
    root.dataset.siteTheme = theme.id;
    
    // Generate CSS variables with site- prefix to avoid next-themes conflicts
    const cssVarString = generateThemeCSS(theme);
    injectThemeCSS(theme.id, cssVarString);
    ...
    if (theme.id !== 'default') {
      ...
      root.dataset.siteTheme = 'default';
    }
  }
}

/**
 * Generate theme CSS with namespaced variables
 * Uses :root[data-site-theme="id"] selector to scope theme
 * 
 * IMPORTANT: Does NOT override base Tailwind/next-themes variables
 * Only adds --site-* prefixed variables for custom theme overlays
 */
function generateThemeCSS(theme: ThemeConfig): string {
  ...
  const cssLines: string[] = [
    ...
    `  /* Custom Theme: ${theme.name} v${theme.version} */`,
    `  /* Colors - prefixed with --site- to avoid next-themes conflicts */`,
    `  /* These supplement, not replace, base color mode variables */`,
  ];
  
  // Add color variables with --site- prefix
  // These are for custom theme elements, not base UI
  Object.entries(colors).forEach(([key, value]) => {
    const cssKey = key.replace(/([A-Z])/g, '-$1').toLowerCase();
    cssLines.push(`  --site-${cssKey}: ${value};`);
  });
  
  // Add typography variables
  cssLines.push(`  /* Typography */`);
  cssLines.push(`  --site-font-heading: ${typography.fontFamily.heading};`);
  cssLines.push(`  --site-font-body: ${typography.fontFamily.body};`);
  cssLines.push(`  --site-font-mono: ${typography.fontFamily.mono};`);
  
  // Add spacing
  cssLines.push(`  /* Spacing */`);
  cssLines.push(`  --site-spacing-scale: ${spacing.scale};`);
  
  // Add effects
  cssLines.push(`  /* Effects */`);
  cssLines.push(`  --site-transition-duration: ${effects.transitionDuration || 200}ms;`);
  
  cssLines.push('}');
  
  // Add a comment explaining usage
  cssLines.push('');
  cssLines.push('/* Usage in components: */');
  cssLines.push('/* Use var(--site-primary) for theme-specific colors */');
  cssLines.push('/* Use var(--primary) for base UI colors that respect light/dark mode */');
...
function injectThemeCSS(themeId: string, css: string): void {
  ...
  const existingStyle = document.getElementById(`site-theme-${themeId}`);
  if (existingStyle) {
